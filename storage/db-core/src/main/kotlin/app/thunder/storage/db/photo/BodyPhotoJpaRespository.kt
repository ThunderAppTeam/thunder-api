...
internal interface BodyPhotoJpaRespository : JpaRepository<BodyPhotoEntity, Long>, KotlinJdslJpqlExecutor

internal fun BodyPhotoJpaRespository.findAllByMemberId(memberId: Long): List<BodyPhotoEntity> {
    return this.findAll {
        select(
...
internal fun BodyPhotoJpaRespository.findAllNotReviewCompleted(): List<BodyPhotoEntity> {
    return this.findAll {
        select(
            entity(BodyPhotoEntity::class)
        ).from(
...
internal fun BodyPhotoJpaRespository.findAllByGender(gender: Gender): List<BodyPhotoEntity> {
